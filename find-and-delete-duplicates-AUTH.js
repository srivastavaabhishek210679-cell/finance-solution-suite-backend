/**
 * Find and Delete Duplicate Reports - AUTHENTICATED VERSION
 * Requires JWT token for API access
 */

const API_BASE = 'http://localhost:3002/api/v1';

// PASTE YOUR TOKEN HERE (get from browser DevTools)
...
// Or pass as command line argument: node script.js YOUR_TOKEN_HERE
const TOKEN = process.argv[2] || JWT_TOKEN;
...
if (!TOKEN || TOKEN === 'PASTE_YOUR_TOKEN_HERE') {
  ...
  console.log('   2. Press F12 → Application → Local Storage');
  console.log('   3. Copy token value\n');
  console.log('Then either:');
  console.log('   - Paste it in line 9 of this script');
  console.log('   - Or run: node find-and-delete-duplicates-AUTH.js YOUR_TOKEN\n');
  process.exit(1);
}
...
async function findDuplicates() {
  ...
  try {
    ...
    let reports = data.data || data.reports || data;
...
async function main() {
  console.log('\n' + '='.repeat(80));
  console.log('  🧹 DUPLICATE REPORT CLEANUP TOOL (AUTHENTICATED)');
  console.log('='.repeat(80) + '\n');
