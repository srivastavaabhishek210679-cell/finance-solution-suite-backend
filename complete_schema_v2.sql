...
COMMENT ON TABLE public.data_lineage IS 'Field-level data provenance for SOX/HIPAA audits Ã¢â‚¬â€ maps source fields to report output fields';
...
COMMENT ON TABLE public.domains IS '[NEW] Normalised domain lookup (Finance, HR, Ã¢â‚¬Â¦); replaces free-text string in reports_master';
...
COMMENT ON TABLE public.tenants IS 'Multi-tenant isolation root Ã¢â‚¬â€ all data scopes by tenant_id';
...
COMMENT ON TABLE public.role_permissions IS '[NEW] M:N junction Ã¢â‚¬â€ which permissions each role carries';
...
COMMENT ON TABLE public.tenant_subscriptions IS 'Full history of plan changes per tenant Ã¢â‚¬â€ current + past subscriptions';
...
COMMENT ON TABLE public.upload_validation_rules IS '[NEW] M:N junction Ã¢â‚¬â€ which rules ran on which upload, with results';
