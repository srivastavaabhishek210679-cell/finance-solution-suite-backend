...
// ══════════════════════════════════════════════════════════════════════════════
// 10. WHATSAPP NOTIFICATIONS via WATI
// ══════════════════════════════════════════════════════════════════════════════
const WATI_API_URL = process.env.WATI_API_URL || '';
const WATI_API_TOKEN = process.env.WATI_API_TOKEN || '';

export async function sendWhatsApp(phone: string, message: string, templateName?: string, params?: string[]) {
  if (!WATI_API_URL || !WATI_API_TOKEN) {
    console.log('[WhatsApp] WATI not configured. Set WATI_API_URL and WATI_API_TOKEN env vars.');
    return false;
  }
  try {
    ...
    const body = templateName ? {
      ...
      broadcast_name: 'deemona_alert',
      parameters: (params || []).map((v, i) => ({ name: `p${i+1}`, value: v }))
    } : { whatsappNumber: phone, messageText: message };
    ...
    console.log(`[WhatsApp] Sent to ${phone}:`, data.result ? 'success' : 'failed');
...
// WhatsApp alert for leave approval
export async function sendLeaveApprovalWhatsApp(managerPhone: string, employeeName: string, leaveType: string, days: number, leaveId: number) {
  const message = `*Deemona ERP — Leave Approval Required*\n\n` +
    `Employee: *${employeeName}*\n` +
    `Leave Type: ${leaveType}\n` +
    `Duration: ${days} day(s)\n\n` +
    `➡️ Approve/Reject: https://finance-frontend-2l6b.onrender.com/leave-mgmt\n` +
    `Leave ID: ${leaveId}`;
  return sendWhatsApp(managerPhone, message);
}

// WhatsApp alert for PO approval
export async function sendPOApprovalWhatsApp(managerPhone: string, supplierName: string, amount: number, poId: number) {
  const message = `*Deemona ERP — PO Approval Required*\n\n` +
    `Supplier: *${supplierName}*\n` +
    `Amount: *₹${amount.toLocaleString('en-IN')}*\n\n` +
    `➡️ Review: https://finance-frontend-2l6b.onrender.com/supply-mgmt\n` +
    `PO ID: ${poId}`;
  return sendWhatsApp(managerPhone, message);
}

// WhatsApp alert for budget overrun
export async function sendBudgetAlertWhatsApp(managerPhone: string, department: string, pct: number) {
  const message = `*⚠️ Deemona ERP — Budget Alert*\n\n` +
    `Department: *${department}*\n` +
    `Utilisation: *${pct}%*\n\n` +
    `Action required — review budget immediately.\n` +
    `➡️ https://finance-frontend-2l6b.onrender.com/budget-mgmt`;
  return sendWhatsApp(managerPhone, message);
}
...
export const startAutomation = () => {
  ...
  // Payroll auto-run — last day of month at 9 AM
  ...
  // Budget overrun check — daily at 7 AM
  cron.schedule('0 7 * * *', checkBudgetOverruns);

  // Low stock check — daily at 7:30 AM
  cron.schedule('30 7 * * *', checkLowStock);

  // Pending leave approvals — weekdays at 9 AM
  cron.schedule('0 9 * * 1-5', checkPendingLeaves);

  // Overdue tickets — daily at 10 AM
  cron.schedule('0 10 * * *', checkOverdueTickets);

  // Warranty expiry — every Monday at 8 AM
  cron.schedule('0 8 * * 1', checkWarrantyExpiry);

  // Overdue risk actions — every Monday at 9 AM
  cron.schedule('0 9 * * 1', checkOverdueRisks);

  // Monthly email report — 1st of month at 6 AM
