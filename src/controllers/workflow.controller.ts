  ...
  async createDefinition(req: Request, res: Response): Promise<void> {
    try {
      const { name, description, category = 'General', trigger_type = 'manual',
              trigger_config = {}, schedule_cron, status = 'active', tenant_id, created_by } = req.body;
      if (!name) { res.status(400).json({ error: 'name is required' }); return; }

      const result = await pool.query(`
        INSERT INTO workflow_definitions
          (name, description, category, trigger_type, schedule_cron, status,
           run_count, success_count, tenant_id, created_by, created_at, updated_at)
        VALUES ($1,$2,$3,$4,$5,$6,0,0,$7,$8,NOW(),NOW())
        RETURNING *
      `, [name, description, category, trigger_type, schedule_cron, status, tenant_id, created_by]);
    ...
    try {
      ...
      const result = await pool.query(`
        SELECT
          ...
          wd.name AS workflow_name, wd.category,
          wi.status, wi.started_at, wi.completed_at,
          wi.error_message, wi.created_at,
          EXTRACT(EPOCH FROM (COALESCE(wi.completed_at, NOW()) - wi.started_at))::integer AS duration_seconds
    ...
    try {
      ...
      const stats = await pool.query(`
        SELECT
          ...
          AVG(EXTRACT(EPOCH FROM (wi.completed_at - wi.started_at)))::integer AS avg_duration_seconds,
      ...
      res.json({
        ...
        successRate: row.total_runs > 0
          ? parseFloat(((row.successful / row.total_runs) * 100).toFixed(1)) : 0,
      });
    } catch (error: any) {
