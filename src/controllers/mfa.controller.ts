...
export const mfaController = {
  sendOTP: async (req: Request, res: Response) => {
    try {
      ...
      await emailService.sendEmail({
        to: email,
        subject: 'Your Finance Suite OTP Code',
        html: 
          <div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto;background:#0f172a;color:#f1f5f9;padding:30px;border-radius:12px;">
            <h2 style="color:#10b981;margin-bottom:20px;">Finance Solution Suite</h2>
            <p style="color:#94a3b8;margin-bottom:20px;">Your One-Time Password (OTP) for login:</p>
            <div style="background:#1e293b;border:2px solid #10b981;border-radius:12px;padding:20px;text-align:center;margin-bottom:20px;">
              <span style="font-size:36px;font-weight:700;color:#10b981;letter-spacing:8px;"></span>
            </div>
  ...
  verifyOTP: async (req: Request, res: Response) => {
    try {
      ...
      if (new Date() > new Date(user.otp_expires_at)) return res.status(400).json({ status: 'error', message: 'OTP expired' });
