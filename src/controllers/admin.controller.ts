...
export const adminController = {
  ...
  getDashboardStats: async (req: Request, res: Response) => {
    try {
      const [users, tenants, reports, orders, invoices, notifications, approvals, webhooks] = await Promise.all([
        pool.query('SELECT COUNT(*) as total, COUNT(CASE WHEN is_active=true THEN 1 END) as active, COUNT(CASE WHEN created_at >= NOW() - INTERVAL ' + "'7 days'" + ' THEN 1 END) as new_this_week FROM users'),
        pool.query('SELECT COUNT(*) as total FROM tenants'),
        pool.query('SELECT COUNT(*) as total FROM report_history'),
  ...
  toggleUserStatus: async (req: Request, res: Response) => {
    try {
      const result = await pool.query('UPDATE users SET is_active=NOT is_active WHERE user_id=' + q(1) + ' RETURNING user_id, email, is_active', [req.params.id]);
  ...
  resetUserPassword: async (req: Request, res: Response) => {
    try {
      ...
      await pool.query('UPDATE users SET password=' + q(1) + ' WHERE user_id=' + q(2), [hashed, req.params.id]);
  ...
  updateSetting: async (req: Request, res: Response) => {
    try {
      ...
      const result = await pool.query('UPDATE admin_settings SET setting_value=' + q(1) + ', updated_by=' + q(2) + ', updated_at=NOW() WHERE setting_key=' + q(3) + ' RETURNING *', [setting_value, userId, req.params.key]);
      res.json({ status: 'success', data: result.rows[0] });
    } catch (e) { res.status(500).json({ status: 'error', message: String(e) }); }
  },
  ...
  getAuditLog: async (req: Request, res: Response) => {
    try {
      ...
      const result = await pool.query('SELECT a.*, u.email FROM audit_logs a LEFT JOIN users u ON a.user_id=u.user_id ORDER BY a.created_at DESC LIMIT ' + q(1) + ' OFFSET ' + q(2), [limit, offset]);
