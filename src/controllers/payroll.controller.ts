...
export const payrollController = {
  ...
  createEmployee: async (req: Request, res: Response) => {
    try {
      ...
      const result = await pool.query(
        'INSERT INTO employees (employee_code, first_name, last_name, email, phone, department, designation, employment_type, date_of_joining, date_of_birth, gender, basic_salary, hra, transport_allowance, medical_allowance, other_allowance, pf_deduction, tax_deduction, other_deduction, bank_account, bank_name, pan_number) VALUES (\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\) RETURNING *',
        [employee_code, first_name, last_name, email, phone, department, designation, employment_type, date_of_joining, date_of_birth, gender, basic_salary, hra, transport_allowance, medical_allowance, other_allowance, pf_deduction, tax_deduction, other_deduction, bank_account, bank_name, pan_number]
      );
      res.json({ status: 'success', data: result.rows[0] });
    } catch (e) { res.status(500).json({ status: 'error', message: String(e) }); }
  },
  ...
  updateEmployee: async (req: Request, res: Response) => {
    try {
      ...
      const result = await pool.query(
        'UPDATE employees SET first_name=\, last_name=\, email=\, phone=\, department=\, designation=\, employment_type=\, basic_salary=\, hra=\, transport_allowance=\, medical_allowance=\, other_allowance=\, pf_deduction=\, tax_deduction=\, other_deduction=\, status=\, updated_at=NOW() WHERE employee_id=\ RETURNING *',
        [first_name, last_name, email, phone, department, designation, employment_type, basic_salary, hra, transport_allowance, medical_allowance, other_allowance, pf_deduction, tax_deduction, other_deduction, status, req.params.id]
      );
      res.json({ status: 'success', data: result.rows[0] });
    } catch (e) { res.status(500).json({ status: 'error', message: String(e) }); }
  },
  ...
  runPayroll: async (req: Request, res: Response) => {
    try {
      ...
      const payrollRun = await pool.query(
        'INSERT INTO payroll_runs (month, year, total_employees, status) VALUES (\, \, \, \) RETURNING *',
        [month, year, employees.rows.length, 'Processing']
      );
      ...
      for (const emp of employees.rows) {
        ...
        await pool.query(
          'INSERT INTO payslips (payroll_id, employee_id, month, year, basic_salary, hra, transport_allowance, medical_allowance, other_allowance, gross_salary, pf_deduction, tax_deduction, other_deduction, total_deductions, net_salary) VALUES (\,\,\,\,\,\,\,\,\,\,\,\,\,\,\)',
          [payrollId, emp.employee_id, month, year, emp.basic_salary, emp.hra, emp.transport_allowance, emp.medical_allowance, emp.other_allowance, gross, emp.pf_deduction, emp.tax_deduction, emp.other_deduction, deductions, net]
        );
      }
