...
router.post('/trigger/:job', async (req: Request, res: Response) => {
  ...
  try {
      ...
      for (const c of result.rows) {
        ...
        for (const u of users.rows) {
          await pool.query(
            ...
            [u.user_id, tenantId, `Contract Expiring in ${days} Days`,
        ...
        if (pct >= 80) {
          ...
          for (const u of users.rows) {
            await pool.query(
              'INSERT INTO app_notifications (user_id,tenant_id,title,message,type,link) VALUES ($1,$2,$3,$4,$5,$6)',
              [u.user_id, tenantId,
               pct >= 100 ? 'Budget Exceeded' : 'Budget Warning',
               `${b.department} has used ${pct}% of budget. Rs.${(parseFloat(b.allocated_amount)-parseFloat(b.spent_amount)).toLocaleString('en-IN')} remaining.`,
      ...
      if (result.rows.length > 0) {
        ...
        for (const u of users.rows) {
          await pool.query(
            'INSERT INTO app_notifications (user_id,tenant_id,title,message,type,link) VALUES ($1,$2,$3,$4,$5,$6)',
            [u.user_id, tenantId,
             `${result.rows.length} Items Low or Out of Stock`,
             `Low stock: ${result.rows.slice(0,3).map((i: any) => i.item_name).join(', ')}${result.rows.length > 3 ? ` +${result.rows.length-3} more` : ''}`,
    ...
    if (job === 'whatsapp-test') {
      const phone = req.body.phone || process.env.TEST_PHONE || '';
      if (!phone) {
        return res.status(400).json({ status: 'error', message: 'Provide phone in request body: {"phone":"919876543210"}' });
      }
      const { sendWhatsApp } = await import('../services/automation.service');
      const message = 'Deemona ERP - WhatsApp notification test successful!\n\nYour automation alerts are now active.\n\nThis message was sent from Deemona Enterprise Finance Suite.';
      const sent = await sendWhatsApp(phone, message);
      return res.json({
        status: sent ? 'success' : 'error',
        message: sent ? `WhatsApp sent to ${phone}` : 'Failed - check TWILIO credentials in Render env vars'
      });
    }

    return res.status(400).json({
      status: 'error',
      message: `Unknown job: ${job}. Use: contract-expiry, budget-overrun, low-stock, pending-leaves, overdue-tickets, whatsapp-test`
    });
